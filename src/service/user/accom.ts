...
usersRouter.get('/me/accommodation', async (req: any, res, next) => {
  try {
...
usersRouter.post('/accommodation', async (req, res, next) => {
  try {
    const accommodation = await new accoModel(req.body).save();
    res.send(accommodation);
  } catch (error) {
    next(error);
  }
...
usersRouter.put('/accommodation/:id', async (req: any, res, next) => {
  try {
...
usersRouter.delete('/accommodation/:id', async (req: any, res, next) => {
  try {
